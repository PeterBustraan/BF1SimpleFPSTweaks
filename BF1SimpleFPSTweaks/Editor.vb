...
Public Class Editor
    ...
    Public Sub removeCFG(ByVal filePath As String)
        'TO-DO Can this action to cut down on code repititon
        Dim curCfg As String = filePath + "user.cfg"
        'Find previous config for preservation
        If (File.Exists(curCfg)) Then
            'Preserve Earlier backups 
            If (File.Exists(curCfg + ".bak")) Then
                File.Copy(curCfg + ".bak", curCfg + "(" + Date.Now.ToString("HHmmss.MMddyyyy") + ").bak")
                My.Computer.FileSystem.DeleteFile(curCfg)
                'Re assure user they aren't loosing anything
                MsgBox("Backing up and removing user.cfg file")
            End If
        End If
    End Sub
End Class
